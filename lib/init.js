...
export function handler(argv) {
  ...
  if (CONFIG_FILE_FORMATS.indexOf(format.toLowerCase()) === -1) {
    process.stderr.write(red('Invalid format: '));
    process.stderr.write(`${white(format)}\n`);
  ...
  const configPath = `${configFile.path}/${configFile.name}.${format.toLowerCase()}`;
